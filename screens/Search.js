...
export default class Search extends React.Component {
    ...
    getHotNews = () => {
        ...
        console.log(today)
        const url = `http://newsapi.org/v2/everything?from=${today}sortBy=popularity&sources=cbc-news,financial-post,google-news-ca,the-globe-and-mail&apiKey=d8cb19fc85ac40f287bf8c1a0ef6fffe`
        fetch(url)
        .then(res => res.json())
        .then(data => {
    ...
    _hotNews = (props) => {
        return (
            ...
            <View style={{position:'relative', borderRadius:4, overflow:'hidden', marginLeft:Dimensions.get('window').width*0.05}}>
                <Image source={{uri:props.hotNews.urlToImage}} style={{height:150, width: Dimensions.get('window').width*0.8}} resizeMode="cover"/>
    ...
    const hotNewsList = this.state.hotNews.map(hotNews => <this._hotNews hotNews={hotNews}/>)
